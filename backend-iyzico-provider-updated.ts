...
class IyzicoProviderService extends AbstractPaymentProvider<IyzicoOptions> {
  ...
  async authorizePayment(
    ...
    if (paymentSessionData.threeDSCompleted || paymentSessionData.authorized || context?.threeDSCompleted || context?.authorized) {
  ...
  async updatePayment(
    ...
    // CRITICAL FIX: Check if payment was authorized (from callback)
    // When callback updates payment session with threeDSCompleted or authorized,
    // this method must return "authorized" status instead of "pending"
    ...
    console.log('[Iyzico] updatePayment: Keeping status as pending', {

        ...
        if (authResult.status === "success") {
          ...
          try {
            ...
            if (session) {
              ...
              // STRIPE-LIKE WEBHOOK APPROACH:
              // Just update the payment session data with 3DS completion markers
              // The provider's updatePayment method will see these flags and return "authorized" status
              // Medusa will automatically call provider's updatePayment when we call updatePaymentSession

              console.log("[Iyzico Callback] Updating payment session with 3DS completion markers...")
              ...
              const updatedSession = await paymentModule.updatePaymentSession({
                ...
                data: {
                  ...session.data,
                  // Add 3DS completion markers that provider's updatePayment will check
                  threeDSCompleted: true,
                  authorized: true,
                  status: "authorized",  // Hint for provider
              ...
              console.log("[Iyzico Callback] Payment session data updated!")
              console.log("[Iyzico Callback] Updated session status:", updatedSession.status)
              console.log("[Iyzico Callback] Updated session data (preview):", JSON.stringify(updatedSession.data).substring(0, 200))

              // No need to call authorizePaymentSession here
              // Frontend will call cart.complete() which triggers authorizePayment
              // Provider's authorizePayment will see threeDSCompleted and return "authorized"
            } else{

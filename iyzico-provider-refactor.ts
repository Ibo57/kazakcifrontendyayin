...
class IyzicoProviderService extends AbstractPaymentProvider<IyzicoOptions> {
   ...
   * STRIPE-LIKE APPROACH: Just check the status, don't reinitialize payment
   */
  async authorizePayment(
    ...
    console.log("[Iyzico] authorizePayment called", {
      sessionDataStatus: paymentSessionData.status,
      threeDSCompleted: paymentSessionData.threeDSCompleted,
      authorized: paymentSessionData.authorized
    })

    // STRIPE-LIKE BEHAVIOR: Just return the current payment status
    // The actual authorization happens in the callback route
    // This method should only read and return status, not modify it

    // If 3DS completed and payment authorized (from callback), return authorized
    if (paymentSessionData.threeDSCompleted || paymentSessionData.authorized) {
      console.log("[Iyzico] Payment already authorized via 3DS callback")
    ...
    // If 3DS HTML exists but not completed yet, return requires_more
    if (paymentSessionData.threeDSHtmlContent) {
      console.log("[Iyzico] 3DS authentication in progress")
    ...
    // Otherwise return current status or pending
    const currentStatus = (paymentSessionData.status as string) || "pending"
    console.log("[Iyzico] Returning current status:", currentStatus)
  ...
  /**
   * OLD authorizePayment - KEPT FOR REFERENCE, DELETE LATER
   * This was too complex and tried to reinitialize payments
   */
  private async OLD_authorizePayment_COMPLEX(
    paymentSessionData: Record<string, unknown>,
    context: Record<string, unknown>
  ) {
  ...
  async updatePayment(
    ...
    // STRIPE-LIKE APPROACH: context IS the data, don't wrap it again!
    // Medusa passes the current payment session data as context
    ...
    // Check if payment was authorized (from callback)
    if (context.threeDSCompleted || context.authorized || context.status === 'authorized') {
      console.log('[Iyzico] updatePayment: Payment authorized via 3DS callback')

      // Don't wrap context in data again - context IS the data!
      return {
        status: 'authorized',
        data: context  // Return context directly to avoid nesting
      }
    }

    console.log('[Iyzico] updatePayment: Status not authorized, returning pending')

    return {
      status: context.status || "pending",
      data: context  // Return context directly
    }
  }

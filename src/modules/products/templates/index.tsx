...
const ProductTemplate: React.FC<ProductTemplateProps> = ({
  ...
  const handleAddToCart = async () => {
    ...
    try {
      ...
      alert("Ürün sepete eklendi! Sepetinizi görüntülemek için üst menüdeki sepet ikonuna tıklayın.")
      // Refresh to update cart count
      window.location.reload()
    } catch (error: any) {
      console.error("Sepete eklenirken hata oluştu:", error)
      if (error.message && error.message.includes("stock location")) {
        alert("Bu ürün şu anda stokta yok veya satış kanalıyla ilişkilendirilmemiş. Lütfen admin panelinden ürün ayarlarını kontrol edin.")
      } else {
        alert("Ürün sepete eklenirken bir hata oluştu: " + (error.message || "Bilinmeyen hata"))
      }
    } finally {
          ...
          <div className="space-y-6">
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 bg-orange-600 hover:bg-orange-700 text-white py-4 px-6 rounded-lg font-semibold text-lg transition-colors flex items-center justify-center gap-2 disabled:bg-gray-400 disabled:cursor-not-allowed"
              >
                {isAddingToCart ? (
                  <>
